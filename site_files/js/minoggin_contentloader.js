document.addEventListener('DOMContentLoaded', function() {
    // Function to update the CSS link to force reload
    function updateCSS() {
        var cssLink = document.getElementById('main-css');
        var href = cssLink.getAttribute('href').split('?')[0]; // Remove old version query
        var version = new Date().getTime(); // Use a timestamp to ensure a unique URL
        cssLink.setAttribute('href', href + '?v=' + version);
    }
    ...
    document.body.addEventListener('click', function(e) {
        ...
        if (target && target.matches('nav a, .related-topics a, .breadcrumb a, .p a')) {
            e.preventDefault();
            ...
            fetch(href)
                ...
                .then(html => {
                    ...
                    updateCSS(); // Update the CSS to ensure styles are refreshed
                })
                .catch(error => {
